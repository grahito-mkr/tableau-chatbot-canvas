...
There's no filter UI inside the extension itself - filtering is done with
Tableau's own native filter cards, the ones you drag onto the dashboard
yourself (right-click a field > Show Filter, same as any other worksheet).
`app/extension/nativeFilters.ts` listens to those via the Extensions API and
feeds them into the assistant automatically:

- `readDashboardFilters()` reads every "categorical" (pick-a-value) and
  "range" (min/max, used for dates and numbers) filter currently applied on
  any worksheet on the dashboard, and converts it into the same filter shape
  `lib/tableauClient.ts` expects.
- `watchDashboardFilters()` subscribes to `FilterChanged` events on every
  worksheet and re-reads the filter list (debounced) whenever anything
  changes, anywhere on the dashboard.
- `app/extension/page.tsx` calls both on load, and re-sends the current
  filters with every "Ask"/"Build Dashboard" call and every `/api/requery`
  refresh of an existing widget - the same `baseFilters` mechanism in
  `lib/agentLoop.ts` used previously, just fed by Tableau's own filters
  instead of a custom UI.

Because the filters already exist as native Tableau objects, they're
persisted by Tableau itself when the workbook is saved - nothing extra is
stored in `tableau.extensions.settings` for this.

Known limitation: "relative date" and "hierarchical" Tableau filter types
aren't translated yet (only "categorical" and "range" are) - if a dashboard
uses one of those, it's silently ignored (logged to the browser console) and
won't scope the assistant's queries.
...
app/extension/nativeFilters.ts     Reads + listens to Tableau's own native dashboard filters
